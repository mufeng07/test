...
object Ip2 {
  def main(args: Array[String]): Unit = {
    ...
    val proviceAndOne: RDD[(String, Int)] = accessLines.map(log => {
      ...
      //查找
      var province = "未知"
      val index = IpUtil.binarySearch(rulesInExecutor, ipNum)
      if (index != -1) {
    ...
    reduced.foreach(tp => {
      ...
      val conn: Connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/spark?charatorEncoding=utf-8", "root", "123456")
      //写入大量数据的时候，有没有问题？
    ...
    reduced.foreachPartition(it => IpUtil.data2MySQL(it))
